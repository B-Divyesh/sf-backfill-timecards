...
- In a fresh live context, the worker became controlling after first visit. After an online reload, setting the context offline and reloading still showed the weekly board, **Add work block**, and **Offline · saved here**, without errors. An isolated local mirror then served cache namespace revisions `backfill-v1.0.3-qa-A` then `...-qa-B` without changing product source: `registration.update()` surfaced **“An updated timecard is ready. Refresh”** with zero console/page errors. The worker precaches the app shell and implements `skipWaiting` and `clientsClaim`.
